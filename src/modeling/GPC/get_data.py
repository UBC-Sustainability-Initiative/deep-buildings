...
def toTensor(trainX, trainY, testX, testY):
    ...
    rescaled_X_test=pd.DataFrame(x_test_scaled, columns=testX.columns, index=testX.index)
    test_x = torch.from_numpy(rescaled_X_test.values).float().cuda()
...
def split_train_test(X, y, pct_train=0.8, month_range=None, test_year=None,
    ...
    # Get data in tensor format required for GP
    trainX, trainY, testX, testY = toTensor(trainX, trainY, testX, testY)
    ...
    if save_data:
        ...
        with open(pname+'trainX.pkl', 'wb') as outfile:
                pickle.dump(trainX, outfile, pickle.HIGHEST_PROTOCOL)
        with open(pname+'testX.pkl', 'wb') as outfile:
                pickle.dump(testX, outfile, pickle.HIGHEST_PROTOCOL)
        with open(pname+'trainY.pkl', 'wb') as outfile:
                pickle.dump(trainY, outfile, pickle.HIGHEST_PROTOCOL)
        with open(pname+'testY.pkl', 'wb') as outfile:
                pickle.dump(testY, outfile, pickle.HIGHEST_PROTOCOL)
